...
// Init es para compatibilidad hacia atrás, pero ahora New() hace todo
func Init() {
	New() // Simplemente llama a New() que ya hace toda la configuración
}
...
func SetReqId(ctx context.Context, id string) context.Context {
	if id == "" {
		id = uid.RandString(uid.CharsetAlphaNum, 20)
	}
